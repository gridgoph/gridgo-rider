  ...
  it("rejects a fake bundle that is missing the configured Clerk value", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        `https://api.gridgo.invalid/ https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png?key=${cartoKey}`,
      );
      ...
      const verified = spawnSync("bash", [join(root, "scripts/verify-release-apk.sh"), apk], {
        ...
        env: {
          ...
          EXPO_PUBLIC_CARTO_API_KEY: cartoKey,
        },
      });
      ...
      expect(verified.stderr).not.toContain(cartoKey);
    } finally {
      rmSync(work, { recursive: true, force: true });
    }
  });

  it("rejects a fake bundle that is missing the configured CARTO key without printing it", () => {
    const work = mkdtempSync(join(root, ".release-verifier-test-"));
    try {
      const bin = join(work, "bin");
      const payload = join(work, "payload");
      ...
      writeFileSync(
        join(assets, "index.android.bundle"),
        `https://api.gridgo.invalid/ ${clerkValue} https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png?key=`,
      );
      const zipped = spawnSync("zip", ["-q", "-r", apk, "assets"], {
        cwd: payload,
        encoding: "utf8",
      ...
      expect(verified.status).toBe(1);
      expect(verified.stderr).toMatch(/CARTO basemap key is not in/i);
      expect(verified.stderr).not.toContain(cartoKey);
    } finally {
      rmSync(work, { recursive: true, force: true });
    }

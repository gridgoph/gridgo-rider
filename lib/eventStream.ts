 ...
 *
 * `lib/alertStream.ts` owns the authenticated XHR transport on native and web,
 * including reconnection. This module only parses frames and computes backoff.
 *
...
export function parseSseChunk(buffer: string): SseParseResult {
  ...
  for (let index = 0; index < buffer.length; index += 1) {
    ...
    const end = index + (char === "\r" && buffer[index + 1] === "\n" ? 2 : 1);
    if (index === lineStart) {
      parts.push(buffer.slice(frameStart, lineStart).replace(/\r\n|\r/g, "\n"));
      frameStart = end;
    }
    lineStart = end;
    index = end - 1;
  }
  const rest = buffer.slice(frameStart);

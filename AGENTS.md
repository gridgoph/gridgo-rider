...
- **The rider's action disc is labelled, which is why its column overhangs on iOS.** `tabBarActionOverhang` — disc + label box + bottom padding against the row. Android: 56 + 16 + 8 = exactly 80, flush. iOS: 44 + 16 + 3 = 63 against 49, so the disc rises **14pt above the row** and is never measured into it (iOS stays on 83pt). Shrinking the disc or dropping the label were the alternatives and both are worse — see the function's comment. Disc diameters match gridgo-client per platform; keep all three apps in step.

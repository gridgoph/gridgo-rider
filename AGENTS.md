...
- **Nothing published is a way in.** The login screen carries no demo credential and pre-fills no field: it used to arrive with `rider@gridgo.local` / `demo` typed in and printed underneath, which on a hosted pilot hands a rider session to anyone who opens the app. Demo accounts still exist; their passwords come from deployment configuration, never the repository. Keep on login only what helps a real rider — who the app is for, the way to create an account, and the API base with its reachability chip.

...
- **Tab bar geometry is per platform, and both platforms have complained.** `tabBarPaddingBottom` in `components/GridgoTabBar.tsx`: iOS takes `insets.bottom` alone (UIKit is 49pt + the 34pt inset with no padding of its own, and React Navigation's own bar does the same — stacking 8 on top is the "sits too high" report), falling back to the design pad when there is no home indicator; Android stacks `insets.bottom + TAB_DESIGN_PADDING`, because MD3's 80dp container sits above the inset and our painted row is 64 (`Math.max` is still banned — it hands Android the bare inset, which read as too tight). Painted heights: iOS 98 / 72, Android 96 / 120. Content row is MD3 80dp; the action column is 56 disc + 16 label box + 8 pad = the same 80. Matches gridgo-client exactly.

...
This app serves `rider`. Authenticated requests select the rider projection with `X-GRIDGO-Role: rider`; `hooks/useClerkSessionBridge.ts` adopts the role-scoped `/auth/me` response, and `store/session.ts` requires its role to be `rider`. Every read and write remains authorized by `gridgo-api`. A refresh-time access rejection blocks Clerk re-adoption and ends the captured Clerk session while preserving the login error.

...
- **Confirmations are native sheets, never a hand-rolled `<Modal>`:** a route with `confirmSheetScreenOptions` from `lib/navigationHeaders.ts` (`presentation: "formSheet"`, `sheetAllowedDetents: "fitToContents"`), body in `components/ConfirmSheetBody.tsx`. That buys platform spring physics, drag-dismiss, the Android back gesture, a real scrim and a screen-reader focus trap for free. Where the screen has already made the choice explicit, prefer inline disclosure — the button names the outcome (`failureOutcomeCommit`) — over a sheet that asks again.
- **Screen furniture:** `ScreenHeader` (title + one control), `EmptyState` (icon + invitation, centred), `Skeleton`/`LoadingCard` (never a bare spinner), `DestinationRow` (label + chevron, not CTA styling), `StickyActionBar` (pinned action on pushed proof screens only — tab screens keep the CTA in the flow so it never stacks under the yellow disc).

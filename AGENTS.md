...
- Files: `lib/push.ts` + `store/push.ts` + `hooks/usePushNotifications.ts` + `components/PushEnableCard.tsx`, and the explainer: `app/push-permission.tsx` + `hooks/usePushPrompt.ts` + `store/pushPrompt.ts`. `POST /devices` and logout `{deviceToken}` live in `lib/api.ts`.
...
- **The OS dialog follows a tap, never a launch.** A signed-in rider on the tabs gets the explainer sheet (what arrives, then "Turn on notifications" / "Not now"); it re-offers no sooner than 7 days after it was last shown (`shouldOfferPushPrompt`), queues behind the update sheet, and on a `blocked` phone (`canAskAgain: false`) its button opens the phone's settings. Between times `PushEnableCard` is the recovery, drawn on Active (the landing screen, guarded by `__tests__/pushLanding.test.ts`), Offers and Alerts. See the login exclusion above. Signed-out may ask *only*, from the card; it is never told it is blocked or that registration failed.
- **Every foreground re-reads permission and registers** (`usePush.resume`), so a rider back from the settings screen is registered at once.
...
- **The app offers its own updates.** On launch and on foreground (throttled 4h) it reads this repo's latest GitHub Release, and when that release's tag suffix is above the installed `versionCode` it opens the `app/app-update.tsx` sheet: "Update now" opens the landing APK, and "Later" quiets that release until the next day. On the first launch of a newer build it says "Update completed" once. The rules are pure in `lib/appUpdate.ts`, and gridgo-client and gridgo-supplier keep the same file, so change all three together. Only its `APP_UPDATE_SOURCE` and body copy are this app's. Dev and Expo Go have no real `versionCode`, so the check is skipped there unless `EXPO_PUBLIC_UPDATE_CHECK_FORCE_VERSION_CODE=<n>` is set when Metro starts. Failure is silence to the rider, but a development build logs every decision as an `[update-check]` line in Metro (`logUpdateCheck`: the build compared, GitHub's answer, the offer, and which gate is holding the sheet); a "Later" is remembered for the rest of that day. The read sends its own `User-Agent`, because GitHub answers a request without one with 403, the same status as its rate limit. The stack re-keys on `rootStackOwner`, so an owner change unmounting the sheet is `interrupted`, never "Later".
